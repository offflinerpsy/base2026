...
## Current Focus: ay50 Live, Source-Review Queue Continuing Under Canonical Gate
...
- Mechanically cleaned and approved 12 local-caption source-review rows across ay47-ay50, cleared only those explicit QA-pass rows back to `transcribed` with `scripts/tiktok-clear-reviewed-source-rows.py`, and deployed `base2026-source-review-local-caption-ay50-20260619`.
- Current source-review audit has 48 private gated rows: 33 local-caption rows requiring source/audio review, 13 audio-backed rows that produced too little/no usable ASR, and 2 rows with no usable local caption/audio yet.
...
- Next transcript action: source-verify the 33 local-caption rows, keep the 13 ASR-too-little rows private until better source/audio evidence exists, and keep the 2 no-source rows private until usable source/audio exists. Do not allow any of those rows into a public release gate before review passes.

Current verified facts:

- Current live release: `base2026-source-review-local-caption-ay50-20260619`.
...
- GPT polish produced 77 polished transcripts: 30 QA-pass rows shipped publicly first, 12 more were mechanically cleaned, approved through the source-review apply gate, cleared with the explicit source-review CSV transition script, and shipped in ay47-ay50; one ASR-recovered source shipped in ay45; one newest-source readiness card shipped in ay46. The remaining rows stayed gated for source/audio review.
...
- Current live public export counts: 1,464 source records, 1,998 passages, 1,630 insight cards, 1,059 public insight cards, 1,521 topics, 1,007 public topics, 10 creators.
- `include_full_transcripts=false`.
- Meilisearch was reindexed with 1,998 public passages.
...
Final ay50 live verification is complete:

- live server current symlink points to `base2026-source-review-local-caption-ay50-20260619`.
...
- ay50 newest-source readiness passed with 0 blocked newest records.
...
2. If the user asks why videos are held, run `python3 scripts/tiktok-source-review-queue.py --limit 25` and process only rows with verified local evidence. Do not bulk-pass the 48 held rows.
