...
| TikTok: `@joshuamaraney` | partially indexed | yes, reviewed export only | ay50 refresh expanded inventory to 638 rows for this creator. Caption availability is weak: 167 queued transcripts and 99 `needs_asr` remain across the active TikTok inventory after refresh. |
| Private Base2026 SEO/GEO/AEO files | local only | no | Do not publish raw project folders. |
| `public-data/tiktok` | generated local export | no git | Latest local export after ay50 refresh: 1209 source records, 1373 passages, 1538 insight cards, 1442 topics, 4 creators, 1097 public insight cards. Public release payload is excerpt-only by default and not committed. Deployed as `base2026-tiktok-refresh-ay50-20260611`. |
| Meilisearch index `base2026_public_tiktok` | deployed runtime | public search only | Search key only; master key private. Reindexed during `base2026-tiktok-refresh-ay50-20260611` deploy with 1373 passages. |
| TikTok transcript queue `config/tiktok-intake-queue.20260608.json` | staged/check-only active | no git unless reviewed | Mac pipeline refresh is operational after runner fixes. Current local inventory: 3008 total rows, 1209 active rows, 167 queued transcripts, 99 `needs_asr`, 941 transcribed rows, 266 queued ASR jobs. |
