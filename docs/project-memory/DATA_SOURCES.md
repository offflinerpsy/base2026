...
| `public-data/tiktok` | generated local export | no git | Latest local export after ay55 full-pipeline refresh: 1214 source records, 1703 passages, 1544 insight cards, 1452 topics, 4 creators, 1103 public insight cards. Public release payload is excerpt-only by default and not committed. Deployed as `base2026-full-pipeline-refresh-ay55-20260612`. |
| Meilisearch index `base2026_public_tiktok` | deployed runtime | public search only | Search key only; master key private. Reindexed during `base2026-full-pipeline-refresh-ay55-20260612` deploy with 1703 passages. |
| TikTok transcript queue `config/tiktok-intake-queue.local.json` | staged/check-only active | no git | Mac pipeline refresh is operational after runner fixes. Current local inventory: 3013 total rows, 1214 active rows, 0 queued transcripts, 0 `needs_asr`, 1211 transcribed rows, 0 queued ASR jobs. ay55 used the ignored local 4-creator runtime config and found 0 new rows. Public-safe `config/creators.example.json` mirrors the 4 public creator sources. |
| TikTok source-review backlog | needs review | no git | 3 older `@tjrobertson52` videos remain marked `needs_source_review`. `scripts/tiktok-source-review-audit.py --probe-network` classifies the current blockers as 2 fallback media files with no audio stream and 1 TikTok IP-blocked source. Source pages without usable public evidence stay `noindex,follow` and are excluded from source listings/sitemaps when no public evidence exists. |
| Private insight-card backfill queue | active local queue | no git | ay52 generated 21 local candidates for the 5 new source records, evidence-verified all 21, promoted 6 public candidates, and retained 15 private `needs_human` candidates. Durability gap closed in ay54: all 21 reviewed/private candidates are archived in ignored `12_knowledge-base/sources/tiktok/insight-candidates/reviewed-candidates.jsonl` and replayed by `build-kb-sqlite.py` during clean rebuilds, while `export-public-tiktok.py` excludes non-public candidates from public JSONL. Current strict review keeps all 15 private: evidence matches, but card copy needs rewrite and/or the source already has the maximum approved candidate cards. |
