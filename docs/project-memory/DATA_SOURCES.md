...
| `public-data/tiktok` | generated local export | no git | Latest local export after ay56 full-pipeline refresh: 1214 source records, 1707 passages, 1544 insight cards, 1452 topics, 4 creators, 1103 public insight cards. Public release payload is excerpt-only by default and not committed. Deployed as `base2026-pipeline-refresh-ay56-20260612`. |
| Meilisearch index `base2026_public_tiktok` | deployed runtime | public search only | Search key only; master key private. Reindexed during `base2026-pipeline-refresh-ay56-20260612` deploy with 1707 passages. |
| TikTok transcript queue `config/tiktok-intake-queue.local.json` | staged/check-only active | no git | Mac pipeline refresh is operational after runner fixes. Current local inventory: 3013 total rows, 1214 active rows, 0 queued transcripts, 0 `needs_asr`, 1213 transcribed rows, 0 queued ASR jobs, and 0 missing polish files. ay56 used the ignored local 4-creator runtime config and found 0 new rows. Public-safe `config/creators.example.json` mirrors the 4 public creator sources. |
