...
| `public-data/tiktok` | generated local export | no git | Latest local export after ay66 full four-creator refresh: 1215 source records, 1708 passages, 1553 insight cards, 1460 topics, 4 creators, 1113 public insight cards, and 1054 public topics. Public release payload is excerpt-only by default and not committed. Deployed as `base2026-full-pipeline-refresh-ay66-20260612`. |
| Meilisearch index `base2026_public_tiktok` | deployed runtime | public search only | Search key only; master key private. Reindexed during `base2026-full-pipeline-refresh-ay66-20260612` deploy with 1708 passages. |
| TikTok transcript queue `config/tiktok-intake-queue.local.json` | staged/check-only active | no git | Mac pipeline refresh is operational after runner fixes. Current local inventory after ay66: 3014 total rows, 1215 active rows, 0 queued transcripts, 0 `needs_asr`, 1214 transcribed rows, 0 queued ASR jobs, and 0 missing polish files. ay66 checked all four configured public creators at `PlaylistEnd=1000` and found 0 added rows. Current triage: 619 review flags, all audio/source-verification. Public-safe `config/creators.example.json` mirrors the 4 public creator sources. |
