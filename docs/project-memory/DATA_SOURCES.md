...
| `public-data/tiktok` | generated local export | no git | Latest local export after ay62 intake refresh and public-text cleanup: 1214 source records, 1707 passages, 1553 insight cards, 1460 topics, 4 creators, 1112 public insight cards, and 1053 public topics. Public release payload is excerpt-only by default and not committed. Deployed as `base2026-intake-refresh-ay62-20260612`. |
| Meilisearch index `base2026_public_tiktok` | deployed runtime | public search only | Search key only; master key private. Reindexed during `base2026-intake-refresh-ay62-20260612` deploy with 1707 passages. |
| TikTok transcript queue `config/tiktok-intake-queue.local.json` | staged/check-only active | no git | Mac pipeline refresh is operational after runner fixes. Current local inventory: 3013 total rows, 1214 active rows, 0 queued transcripts, 0 `needs_asr`, 1213 transcribed rows, 0 queued ASR jobs, and 0 missing polish files. ay62 checked the latest 160 posts for each of the 4 configured creators and found 0 new rows. Public-safe `config/creators.example.json` mirrors the 4 public creator sources. |
