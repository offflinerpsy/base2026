...
| --- | --- | --- | --- |
| Public release | live | `base2026-source-intelligence-contract-ay54-20260619` | newest-source readiness clean; next data change must use the release gate |
| Public export | live | 1,476 sources; 2,016 passages; 1,631 insights; 1,060 public insights; 1,522 topics; 1,008 public topics; 10 creators | export policy and release contract before every package |
| Meilisearch | live | `base2026_public_tiktok`, 2,016 passages | reindex only when data changes |
| TikTok pipeline | operational | AI Recommends Solutions pass processed in `hermes-polish-20260618-ai-recommends`; 77 polished rows reviewed; source-review cleanup is live through ay53; ay54 fixed Source Intelligence/Q&A output for one reviewed source; 36 source-review rows remain private/gated; `needs_asr=0` | run `python3 scripts/tiktok-source-review-queue.py --limit 25`; use release gate; never bypass newest-source readiness or transcript QA |
...
| DATA-01 | Process fresh TikTok batch | Main Codex | completed/live | AI Recommends Solutions ay53 batch includes explicit local-caption source-review cleanup; ay54 adds reviewed Source Intelligence/Q&A contract fix; readiness blockers resolved with reviewed exact-evidence insights; ay54 deployed |
| QA-01 | Verify live ay54 release | Main Codex | completed | newest-source readiness/export/contract clean; Meili has 2,016 passages; direct source URL smoke clean; full mobile visual QA passed 78/0 |
| MEM-01 | Update project memory after release | Main Codex | completed | handoff, next action, data sources, status board, prompt log, deployment log, project state, deployment runbook, and active queue point at ay54 |
...
| QA-02 | Resolve private source-review queue | Main Codex + review worker | active | use `scripts/tiktok-source-review-queue.py`; 21 local-caption rows need source/QA review, 13 audio-backed rows need better source/audio after too-little ASR, 2 rows stay private until source/audio exists |
