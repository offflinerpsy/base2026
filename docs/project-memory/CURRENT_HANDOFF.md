...
- Current live export: 1,464 source records, 1,998 passages, 1,630 insight cards, 1,059 public insight cards, 1,521 topics, 1,007 public topics, 10 creators.
- Current policy: `include_full_transcripts=false`.
- Current Meilisearch index: `base2026_public_tiktok`, 1,998 public passages.
...
- Mechanically cleaned twelve local-caption source-review rows across ay47-ay50, approved them through `scripts/tiktok-qa-review-apply.py`, cleared only those explicit QA-pass rows back to `transcribed` with `scripts/tiktok-clear-reviewed-source-rows.py`, and deployed `base2026-source-review-local-caption-ay50-20260619` through the canonical release gate. The private gated queue is now 48 rows: 33 local-caption review rows, 13 audio-backed too-little/no-speech rows, and 2 rows with no usable local caption/audio.
...
- ay50 source-review release is live: symlink points to `base2026-source-review-local-caption-ay50-20260619`; public export policy, release contract, newest-source readiness, live SEO crawl gate, and mobile visual QA pass.
...
- The AI Recommends Solutions creator pass is closed in the live ay50 release. The Hermes batch processed 77 videos: 30 passed immediately, 12 local-caption rows were mechanically cleaned and approved through `tiktok-qa-review-apply.py` plus `tiktok-clear-reviewed-source-rows.py`, 1 ASR-recovered row passed later, and ay46 added 1 exact-evidence readiness card. Current private inventory has 0 `needs_asr` rows and 48 `needs_source_review` rows: 33 local-caption review rows, 13 audio-backed rows with too little/no usable ASR, and 2 rows without usable local caption/audio. Those require source/audio verification before public export.
