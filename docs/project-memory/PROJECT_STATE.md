...
- Current transcript polish status has 1213 transcribed/clean/polished transcripts, 0 missing polished files, and 777 historical `needs_review` QA flags after ay62 resolved one additional mojibake QA row. `scripts/tiktok-qa-triage.py` now categorizes the remaining debt as 575 audio-verification rows, 193 entity/spelling rows, and 9 human text-review rows. This remains controlled review debt; public export is still excerpt-only and does not expose full transcripts.
