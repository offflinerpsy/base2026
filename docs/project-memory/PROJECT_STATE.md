...
- latest deployed release: `base2026-pipeline-refresh-ay58-20260612`
...
- `base2026-pipeline-refresh-ay58-20260612` is live after a full four-creator TikTok refresh checked the latest 80 public posts per configured creator and found 0 new videos, 0 queued transcripts, 0 `needs_asr`, and 0 missing polish files.
- Current ay58 public export is excerpt-only with 1214 source records, 1707 passages, 1552 insight cards, 1111 public insight cards, 1460 topics, and 1052 public topics. Meilisearch reindexed 1707 passages.
...
- Current transcript polish status has 1213 transcribed/clean/polished transcripts, 0 missing polished files, and 786 historical `needs_review` QA flags after ay58 passed 8 clean human-text technical artifact rows and kept 17 source-sensitive rows in review. `scripts/tiktok-qa-triage.py` now categorizes the remaining debt as 576 audio-verification rows, 193 entity/spelling rows, and 17 human text-review rows. This remains controlled review debt; public export is still excerpt-only and does not expose full transcripts.
- Current private insight-card candidate queue was reduced by the ay57/ay58 review pass: 8 candidates were rewritten, exact-evidence verified, imported as approved, archived for clean-rebuild replay, and exported publicly; 5 old candidates were rejected; 8 old rows were resolved as superseded by rewritten approved cards; and 2 remain `needs_human` for source/audio verification. Public export still excludes private candidates.
...
- Live ay58 QA passed: `/knowledge/`, static endpoints, public export policy, clean SQLite rebuild replay, controller doctor, publication boundary audit, GitHub metadata validation, and full mixed mobile visual QA with 66 checks and 0 failures.
