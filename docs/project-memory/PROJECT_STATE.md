...
- `base2026-modal-meta-controls-ay32-20260610` is live after moving source-modal policy/platform/language metadata into the sticky right-side control area below the modal action buttons.
- Live modal meta controls QA passed: CSS/JS cache-bust `20260610-modalmeta2`; header meta parent is `.transcript-dialog-controls`; header has 3 meta cards; modal body has 0 policy grids; sticky header remains stable during body scroll; desktop/mobile checks have no horizontal overflow or console errors.
